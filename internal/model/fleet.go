...
type UnitStatusRow struct {
	...
	Code        string     `json:"code" gorm:"column:unit_code;uniqueIndex"`
...
type FleetSettingUnit struct {
	FleetSettingID uint   `gorm:"column:fleet_setting_id;primaryKey"`
...
type FleetAlloc struct {
	...
	FlID   uint              `json:"flId" gorm:"column:fleet_id"`
	Digger string            `json:"digger" gorm:"column:digger_code"`
	Loc    string            `json:"loc" gorm:"column:location"`
	Bus    string            `json:"bus" gorm:"column:bus_code"`
...
type FleetAllocOperator struct {
	AllocationID uint   `gorm:"column:allocation_id;primaryKey"`
	UnitCode     string `gorm:"column:unit_code;primaryKey"`
	OperatorNIK  string `gorm:"column:operator_nik"`
}

func (FleetAllocOperator) TableName() string { return "fleet_allocation_operators" }
