...
CREATE TABLE IF NOT EXISTS master_entries (
    ...
    -- category_key matches FE MdCat (dept, pos, simper, mess, etc.)
...
CREATE TABLE IF NOT EXISTS roster_files (
    -- id = key matching FE RosterMeta.key (e.g. "jul", "jul-plant")
    id VARCHAR(50) PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS roster_schedules (
    id SERIAL PRIMARY KEY,
    roster_file_id VARCHAR(50) REFERENCES roster_files(id) ON DELETE CASCADE,
    employee_nik VARCHAR(50) REFERENCES employees(nik) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS unit_statuses (
    code VARCHAR(50) PRIMARY KEY REFERENCES units_db(code) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS unit_status_histories (
    id SERIAL PRIMARY KEY,
    unit_code VARCHAR(50) REFERENCES unit_statuses(code) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS fleet_setting_units (
    fleet_setting_id VARCHAR(50) REFERENCES fleet_settings(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS fleet_allocations (
    ...
    -- shift matches FE: siang|malam (FE uses pagi/malam — use malam/siang)
    shift VARCHAR(20) NOT NULL CHECK (shift IN ('siang', 'malam')),
    fleet_id VARCHAR(50) REFERENCES fleet_settings(id) ON DELETE CASCADE,
    digger_code VARCHAR(50) NOT NULL,
    location VARCHAR(150),
    bus_code VARCHAR(50),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE IF NOT EXISTS fleet_allocation_operators (
    allocation_id VARCHAR(50) REFERENCES fleet_allocations(id) ON DELETE CASCADE,
    unit_code VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS audio_schedule_displays (
    audio_id VARCHAR(50) REFERENCES audio_schedules(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS display_devices (
    ...
    fleet_id VARCHAR(50) REFERENCES fleet_settings(id) ON DELETE SET NULL,
